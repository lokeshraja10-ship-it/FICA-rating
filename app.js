...
(function () {
  ...
  function CarromRatings() {
    ...
    async function refresh() {
      try {
        const [p, m, t] = await Promise.all([fsGetList("players"), fsGetList("matches"), fsGetList("tournaments")]);
    ...
    return h(
      ...
      h(
        ...
        !loaded
          ...
          : view === "match"
          ? h(MatchView, { players, savePlayers, matches, saveMatches, tournaments, saveTournaments, isAdmin, onViewRankings: () => setView("players") })
          : view === "history"
   ...
   * New Match
   * ==================================================================== */
  function MatchView({ players, savePlayers, matches, saveMatches, tournaments, saveTournaments, isAdmin, onViewRankings }) {
    ...
    const [matchType, setMatchType] = useState("friendly"); // 'friendly' | 'tournament'
    ...
    // In tournament mode, keep sel.blackA/B and whiteA/B in sync with whichever
    // teams are picked, so all the existing scoring logic downstream (which
    // only ever reads `sel`) works completely unchanged either way.
    useEffect(() => {
      if (matchType !== "tournament" || !selectedTournament) return;
    ...
    }, [matchType, tourId, teamBlackId, teamWhiteId]);
    ...
    function canStart() {
      if (matchType === "tournament") {
        return !!tourId && !!teamBlackId && !!teamWhiteId && teamBlackId !== teamWhiteId;
      }
    ...
    function start() {
      ...
      setHistory([]);
      setSummary(null);
      if (matchType === "tournament" && selectedTournament) {
        const teamBlack = selectedTournament.teams.find((t) => t.id === teamBlackId);
        const teamWhite = selectedTournament.teams.find((t) => t.id === teamWhiteId);
        setMatchMeta({
    ...
    function computeFinishState() {
      const blackFinished = coinsUsed.black >= MAX_COLOR_COINS && coinsUsed.red >= MAX_QUEEN;
      const whiteFinished = coinsUsed.white >= MAX_COLOR_COINS && coinsUsed.red >= MAX_QUEEN;
      if (blackFinished && !whiteFinished) return "black";
      if (whiteFinished && !blackFinished) return "white";
      return null; // neither finished yet, or both did simultaneously (ambiguous - fall back to points)
    }
        ...
        (() => {
          ...
          return (
            ...
            h(
              ...
              "\u{1F3C6} ",
              liveFinisher === "black" ? "\u26AB Team Black" : "\u25CB Team White",
              " emptied their coins \u2014 tap Finish match to record the win!"
            )
          );
        })(),
        h(
    ...
    return h(
      "div",
      { className: "panel-card" },
      isAdmin && h(TournamentManager, { players, tournaments, saveTournaments }),
      ...
      canStart() && h(OddsPanel, { black: [playerById(sel.blackA), playerById(sel.blackB)], white: [playerById(sel.whiteA), playerById(sel.whiteB)] }),
      h(
        "button",
