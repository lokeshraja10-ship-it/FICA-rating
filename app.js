...
(function () {
  ...
  function MatchView({ players, savePlayers, matches, saveMatches, tournaments, saveTournaments, isAdmin, onViewRankings }) {
    ...
    const [foulCredits, setFoulCredits] = useState({ black: 0, white: 0 }); // how many of that color's coins came from the OPPONENT's foul, not this team's own pots
    ...
    function start() {
      ...
      setFoulCredits({ black: 0, white: 0 });
      setHistory([]);
      setSummary(null);
    ...
    function undoLast() {
      ...
      if (last.statKey === "fouls") {
        setFoulCredits((f) => ({ ...f, [last.coinsUsedKey]: Math.max(0, f[last.coinsUsedKey] - 1) }));
      }
      setRawStats((s) => ({
        ...s,
        [last.playerId]: {
    ...
    function handleFoul(playerId, team) {
      ...
      setFoulCredits((f) => ({ ...f, [oppColor]: f[oppColor] + 1 }));
      addPoints(playerId, -2);
    ...
    function computeFinishState() {
      ...
      //   - If any of that color's coins came from the OPPONENT's foul, this
      //     team wins outright, queen or no queen (not their fault it emptied).
      //   - Otherwise (all 9 were this team's own legitimate pots): if the
      //     queen has been taken, this team wins normally. If the queen is
      //     still on the board, they ran out without covering it - so the
      //     OPPONENT wins instead.
      function evaluate(color, oppColor) {
        if (coinsUsed[color] < MAX_COLOR_COINS) return null;
        if (foulCredits[color] > 0) return color;
