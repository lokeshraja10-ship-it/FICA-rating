...
(function () {
  ...
  function MatchView({ players, savePlayers, matches, saveMatches, onViewRankings }) {
    ...
    const [firstFinisher, setFirstFinisher] = useState(null); // 'black' | 'white' | null - locks in once a team empties their coins with the queen accounted for
    ...
    // whoever has more points. Locks in the first team to reach it.
    useEffect(() => {
      if (!started || firstFinisher) return;
      ...
      if (blackFinished) setFirstFinisher("black");
      else if (whiteFinished) setFirstFinisher("white");
    }, [coinsUsed, started, firstFinisher]);

    const blackTotal = (pts[sel.blackA] || 0) + (pts[sel.blackB] || 0);
    const whiteTotal = (pts[sel.whiteA] || 0) + (pts[sel.whiteB] || 0);

    function finish() {
      // Real carrom win condition: whoever empties their 9 coins first (with the
      ...
      const blackWon = firstFinisher ? firstFinisher === "black" : blackTotal > whiteTotal;
      const whiteWon = firstFinisher ? firstFinisher === "white" : whiteTotal > blackTotal;
      const isDraw = !firstFinisher && blackTotal === whiteTotal;
      ...
      return h(
        ...
        firstFinisher &&
          h(
            ...
            "\u{1F3C6} ",
            firstFinisher === "black" ? "\u26AB Team Black" : "\u25CB Team White",
