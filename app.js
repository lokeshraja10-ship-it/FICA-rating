...
(function () {
  ...
  // Safe getters — older player records won't have these fields yet.
  function coinsOf(p) { return p.coins || 0; }
  function redsOf(p) { return p.reds || 0; }
  function foulsOf(p) { return p.fouls || 0; }
    ...
    return h(
      ...
      h(
        ...
        h(
          ...
          h(Stat, { label: "Coins", value: coinsOf(player) }),
          h(Stat, { label: "Reds (Queens)", value: redsOf(player) }),
          h(Stat, { label: "Fouls", value: foulsOf(player) })
        )
      )
    );
  }
  ...
  function MatchView({ players, savePlayers, matches, saveMatches, onViewRankings }) {
    ...
    function bumpTally(playerId, key) {
      setTally((t) => ({ ...t, [playerId]: { ...t[playerId], [key]: (t[playerId] ? t[playerId][key] : 0) + 1 } }));
    }
    ...
    function finish() {
      ...
      const nextPlayers = players.map((p) => {
        ...
        const t = tally[p.id] || { coins: 0, reds: 0, fouls: 0 };
        return {
          ...p,
          ...
          coins: coinsOf(p) + t.coins,
          reds: redsOf(p) + t.reds,
          fouls: foulsOf(p) + t.fouls
        };
      });
      const record = {
        id: uid(),
        ...
        black: [sel.blackA, sel.blackB].map((id) => ({
          ...
          points: pts[id] || 0,
          delta: deltas[id],
          coins: (tally[id] || {}).coins || 0,
          reds: (tally[id] || {}).reds || 0,
          fouls: (tally[id] || {}).fouls || 0
        })),
